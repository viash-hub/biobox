...
# --- Test Case 5: 64-bit index file names ---
log "Starting TEST 5: BWA MEM with a 64-bit-named index"

# `bwa index -6` inserts `.64` before every extension, so the index base name is
# `reference.fasta.64` here. The prefix has to be taken from the index files
# themselves rather than guessed by stripping a FASTA extension, and the
# reference FASTA sitting alongside the index must not confuse that.
log "Building BWA index with 64-bit file names..."
mkdir -p "$test_data_dir/index_64"
cp "$test_data_dir/reference.fasta" "$test_data_dir/index_64/"
bwa index -6 "$test_data_dir/index_64/reference.fasta" >/dev/null 2>&1
check_file_exists "$test_data_dir/index_64/reference.fasta.64.bwt" "64-bit-named index file"

log "Executing $meta_name with a 64-bit-named index..."
"$meta_executable" \
  --index "$test_data_dir/index_64" \
  --reads1 "$test_data_dir/reads_single.fastq" \
  --output "$meta_temp_dir/index_64.sam"

log "Validating TEST 5 outputs..."
check_file_exists "$meta_temp_dir/index_64.sam" "64-bit-index SAM output"
check_file_not_empty "$meta_temp_dir/index_64.sam" "64-bit-index SAM output"
check_file_contains "$meta_temp_dir/index_64.sam" "@SQ" "64-bit-index SAM output"
...
if "$meta_executable" \
  --index "$test_data_dir/index/reference.bwt" \
  --reads1 "$test_data_dir/reads_single.fastq" \
...
# A dangling '.bwt' symlink must not be counted as a second index: doing so
...
ln -s "does_not_exist.bwt" "$test_data_dir/dangling_index/dangling.bwt"
